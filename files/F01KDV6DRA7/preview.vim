...
function! candle#preview#filename(filename, ...) abort
  if has('nvim') && executable('bat')
    let l:args = get(a:000, 0, {})
    return candle#preview#command(printf('bat --style=plain --color=always --paging=never --line-range=%s: --highlight-line=%s %s',
...
function! candle#preview#command(cmd) abort
  if has('nvim')
    ...
    call s:Buffer.do(s:term, { -> termopen(a:cmd, { 'stdout_buffered': v:true }) })
